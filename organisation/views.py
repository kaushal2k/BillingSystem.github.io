    ...
    if request.method == "POST":
        try:
            ...
            if m.password_2 == request.POST['password']:
                ...
                return HttpResponse("<h2><a href='' style='color:red;'>You have entered wrong password!!</a></h2>")
        except:
            return HttpResponse("<h2><a href='' style='color:red;'>User is not Registered!!!</a></h2>")
...
def index(request):
    if 'org' in request.session:
        ...
        return render(request, 'index.html',
                      {'org': org.company_name, 'cust_count': cust_count, 'invo_count': tot_invo})
    else:
        return redirect('login')
...
def DetailPage(request, id):
    if 'org' in request.session:
        ...
        return render(request, 'DetailPage.html',
                      {'org': org.company_name, 'cust': cust_data, 'invo': invo, 'prod': prod, 'tot': tot})
    else:
        return redirect('login')
...
def InvoiceUpdate(request, invo_up):
    if 'org' in request.session:
        ...
        if request.POST:
            print(request.POST.get('Invoice_num'))
            invo.companyName = org
            invo.Cust_name = CustomerDetails.objects.get(cust_name=request.POST['Cust_name'])
            invo.Invoice_type = request.POST['Invoice_type']
            invo.Invoice_num = request.POST.get('Invoice_num')
            invo.Date = request.POST['Date']
            invo.Dispatch_through = request.POST['Dispatch_through']
            invo.Due_date = request.POST['Due_date']
            invo.Bank = request.POST['Bank']
...
def ProductForm(request):
    if 'org' in request.session:
        ...
        if request.POST:
            ...
            prod.Total = request.POST.get('Total')
            ...
            if a.Qty >= int(request.POST['Qty']):
                ...
                return render(request, 'productform.html',
                              {'org': org.company_name, 'invo': invo, 'prod': prod2, "m": "Quantity Not Available"})
        ...
        if request.POST:
            ...
            if r >= float(request.POST['Qty']):
                ...
                return render(request, 'updateproduct.html',
                              {'org': org.company_name, 'invo': invo, 'data': prod, "m": "Quantity Not Available"})
